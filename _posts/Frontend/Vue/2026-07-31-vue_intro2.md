...
전부 외울 필요는 없고, 아래 문법을 읽고 간단하게 사용할 수 있으면 된다.
...
`map`은 배열을 변환하고, `filter`는 조건에 맞는 항목만 남기며, `find`는 조건에 맞는 첫 번째 항목을 찾는다. 상품 총합처럼 여러 값을 하나로 합칠 때는 `reduce`를 사용한다.
...
상태에서 새로운 값을 계산하면 `computed`, 사용자 행동을 처리하면 일반 함수를 사용한다.
...
여기까지 이해했으면 이제부터는 문법 자체보다, 어떤 상태를 만들고 어떤 흐름으로 화면에 연결할지 생각하면 된다.
